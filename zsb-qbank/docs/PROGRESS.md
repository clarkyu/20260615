...
## 环境变量三方一致性(2026-09-17)

一个环境变量要对三处:**代码里读它 → `.env.example` 里声明 → compose 往容器里转发**。

逐条对完的结果:**三处齐的,没有缺陷**。代码里读的 18 个运行时变量(含 `getAiConfig` /
`getOidcConfig` 那种 `env.X` 读法的 AI_* 与 CASDOOR_*),`.env.example` 全有、compose 全转发。

但这个不变量值得守住 —— 漏掉第三处是最阴的一种:变量在 `.env` 里填了、看着像生效了,
可 compose 不传进容器,生产上那个旋钮就是**静默失效**的。日志不报错,行为只是「没按预期变」。

新增 `tests/config/env-parity.test.ts`(8 条)。变异验证:

| 变异 | 期望 | 实际 |
| --- | --- | --- |
| compose 漏转发 `AI_BATCH` | 红 | ✅ |
| `.env.example` 漏掉 `AI_TIMEOUT_MS` | 红 | ✅ |
| `.env.example` 留没人读的陈货 | 红 | ✅ |
| 扫描器只认 `process.env.X`(漏掉 `env.X`) | 红 | ✅ 三条防空断言的全红 |

### 一条被变异验证抓出来的自己的毛病

原本有条断言叫「每个变量都有注释(硬约束 9)」。删掉组内某一行注释后**它没红** ——
`.env.example` 是「一段注释管一组变量」的写法,规则看的是分组,抓不到「变量被塞进已有分组、
自己没被提到」。**断言名比它的本事大**,这比没有这条断言更糟:会让人以为这一面已经守住了。
改名为「没有变量是孤零零冒出来的」,注释里写明它查不出什么,再用对得上的变异重验通过(D72)。

### 门禁

- `pnpm lint` / `npx tsc --noEmit` 干净;`pnpm test` **511 passed**(原 503 + 8)
- 不动 `src/`,构建与首屏预算不受影响

## 登出:先有入口,再谈单点登出(2026-09-17)

这一轮起因是三条搁置事项。第二条一查就发现不是「还差一点」,是个真缺口。

### 一个登不出去的应用

`/api/auth/logout` 有接口;SW 有「见到 `/api/auth/*` 就清掉带个人数据的缓存」的逻辑,
注释写得很清楚(「登出请求本身失败也要清:本地至少不该继续留着上一个人的卷子」);
`tests/e2e/offline.spec.ts` 还专门有一条「登出后,带个人数据的缓存被清空」,用
`page.evaluate(() => fetch('/api/auth/logout', {method:'POST'}))` 验过。

**但产品里没有任何界面调用它** —— 全站搜不到一个「退出」。三样东西各自都对,合起来是个
登不出去的应用:共用手机、机房电脑上,上一个人的会话一直挂着,下一个人顶着他的身份做题。
那条 e2e 用例之所以要用 `fetch` 而不是点按钮,原因就在这儿,只是当时没人追问。

学生端首页与教师端顶栏各补一个入口。作答页不放:误触一下就是把人踢出考试。

### 只清本地不算退出

身份服务器那边的会话还在:点「退出」再点「登录」,Casdoor 不会再问一次密码,直接把同一个人
登回来 —— 在共用设备上等于没退。所以接了 OIDC RP-Initiated Logout,三个取舍见 D74:
端点问发现文档要(不写死 Casdoor 的路径)、先清本地再问身份服务器(问不到也得退得出去)、
登出端点必须与 issuer 同源(那地址会原样 302 给学生的浏览器,配错一个字就是开放重定向)。

`id_token` 因此要存进会话当 `id_token_hint`,是 §9.5 的一处例外;**超过 2048 字符的不存** ——
会话 Cookie 有 4KB 上限,为了登出方便把登录搞坏不划算。

新增 `tests/auth/logout.test.ts`(19 条,失败路径比成功路径多)与 `tests/e2e/logout.spec.ts`
(从「界面上点得到」验起,而不是从接口验起)。变异验证:

| 变异 | 期望 | 实际 |
| --- | --- | --- |
| 去掉「登出端点必须与 issuer 同源」 | 红 | ✅ 5 条 |
| 发现文档取失败也写进缓存(抽风一次钉死整个进程) | 红 | ✅ |
| 不给发现文档的 fetch 传 signal(超时那根线断掉) | 红 | ✅ |
| `id_token` 不做大小判断 | 红 | ✅ |
| 登出只跳转、不销毁会话(界面看着退了,会话还在) | 红 | ✅ e2e 2 条 |

> 第二个变异第一次是我写坏的:只改了读端,而失败路径本来就不写缓存 —— 等于什么都没变异,
> 自然「全绿」。重写成真能复现该 bug 的版本才验出红。**变异没复现 bug,证明不了任何事。**

### 顺带:D56 那个「留给 clark 定」的取舍,其实是选项没列全

D56 把空位芯片的 40px 摆成两条路:维持现状,或把短文行距撑到 ≥46px(段落长一大截)。
这轮用体检用例同一套探针在 iPhone 13 仿真上逐个量,发现第三条:**给芯片加 6px 上下外边距**。
芯片是 `inline-flex`,外边距会算进行盒高度,于是只有含空位的行变高,纯文字行一点不动。

| 方案 | 20 个空位达标 | 短文高度 |
| --- | --- | --- |
| 现状 | 19 / 20(第 23 空 40px) | — |
| 芯片加 6px 上下外边距 | **20 / 20**(最小 45px) | **+14%** |
| 撑行距到 46px(D56 设想的那条) | 20 / 20 | +48% ~ +52% |

代价从 +50% 降到 +14%,硬约束 5 又白纸黑字写着 44px,就没有理由再欠着(D73)。
`touch-targets.spec.ts` 里 `[data-blank]` 的 40px 特例下限一并撤掉 —— 那种特例下限本身
就是 D72 说的隐患。变异验证:撤掉 `my-1.5` → 体检用例当场变红。

第三条(SW Background Sync)判定仍然不做,理由从代码注释提到台账(D75),并写明重议的条件是
真实数据而不是想象。

### 门禁

- `pnpm lint` / `npx tsc --noEmit` 干净;`pnpm test` **530 passed**(原 511 + 19,对真库跑的全套)
- `pnpm build` 通过;e2e 双视口实跑(登出 2 条 + 触控体检 2 条 × 2 project)
- 未接 Casdoor 实例:与真身份服务器的登出回跳需 clark 在 Casdoor 里加一条 post-logout 回跳地址
  后真机验一次(RUNBOOK §5 已写明)

## 把登出那个 bug 的形状变成 CI 检查,当场又抓出一个(2026-09-17)

#496 补登出入口时,那个 bug 的形状值得记一笔:**接口、逻辑、测试全都对,就是没人调用它**。
每个零件的用例都是绿的,所以类型、lint、单测一个都拦不住。既然这种 bug 存在过一次,
先查全站还有没有第二处,再决定守什么。

### 查:39 条路由,5 条搜不到调用方

逐条核对(一次 grep 不足以下结论,拼接式 URL 会漏):

- `/api/health` —— Docker 健康检查在用,GET,不算。
- `GET /api/teacher/grading/queue`、`GET /api/teacher/assignments/:id/stats`、
  `GET /api/teacher/classes/:id` —— SPEC §9.4 规定的接口面,页面是服务端组件直接查库渲染的,
  没有界面调用方**正常**。
- `POST /api/teacher/items/:id/explain` —— **第二处**。接口在、`explain-item.md` 提示词在、
  worker 在、「同一小题不重复入队」的去重在、测试在,SPEC §9.4 也列着「解析生成」。
  `enqueueExplainJob` 全仓库只有这条孤儿路由调用,而这条路由没有任何界面调用。
  **老师在题库页点不到「生成解析」。**

最后一道门还挺隐蔽:`ItemTools` 开头有 `if (!variantable) return null` —— 就算当初加了按钮,
非变式题型也照样看不到。补入口时把这个早退去掉了,解析对每种题型都有意义。

### 守:只查写接口

`tests/config/route-reachability.test.ts`。一个写接口就是一个用户动作,没人调用就是一个
用户做不到的动作;只读接口不算(理由见 D76)。豁免名单目前是空的。

### 这条用例第一版是个只会亮绿灯的东西

写完就绿。跑变异才发现:把「生成解析」和「退出登录」两个入口**都删掉,它照样全绿** ——
扫描器把注释也当成了调用方,而这两个文件的注释里恰好都写着各自的接口路径,自己把自己喂绿。
**一条专门为这个 bug 写的用例,抓不住这个 bug。** 加 `stripComments` 后才验出红。

顺带还揪出自己一条名不副实的断言:「通配不跨引号」用的是 `auth/logout`,它没有动态段,
根本没走到通配那一步(D72 的老问题,又犯了一次)。

| 变异 | 期望 | 实际 |
| --- | --- | --- |
| 拿掉「生成解析」入口 | 红 | ✅ |
| 拿掉「退出登录」入口(#496 那个 bug) | 红 | ✅ |
| 扫描器不再剥注释(回到第一版) | 红 | ✅ |
| 通配放宽成 `.+`(跨引号乱认) | 红 | ✅ |
| e2e:按钮不渲染 | 红 | ✅ |
| e2e:URL 拼错一个字(按钮在,链路断) | 红 | ✅ |

静态扫描只证明「有代码引用这个接口」——按钮渲染不出来、URL 拼错、被早退挡掉,它都看不见。
所以另加 `tests/e2e/teacher-item-tools.spec.ts` 点真按钮:POST → 入 ai_jobs → 轮询 →
界面说「失败:AI 未配置」。本地与 CI 都没配 AI,那正好是要验的:链路通了,失败原因说清楚了。

### 顺带发现,没在本轮改

教师端试卷列表在 390px 视口下,表格单元格会盖住「查看整卷」链接,点不中。教师端按 SPEC §8
是桌面优先,不算违反硬约束 5(那条只管学生端),所以没动它 —— 但老师用手机查东西时会碰上。
本轮的 e2e 改成直接进整卷页,不让这条用例替教师端的取舍背锅。

### 门禁

- `pnpm lint` / `npx tsc --noEmit` 干净;`pnpm test` **534 passed**(原 530 + 4)
- `pnpm build` 通过;e2e 对 **standalone 产物**双视口跑

## 教师端宽表格在手机上点不动(2026-09-17)

#497 里顺手记了一条「没在本轮改」:教师端试卷列表在 390px 下,单元格盖住「查看整卷」。
这轮把它做掉,顺带把测量这件事本身摔了三次。

### 先把「写了没接通」那条矿脉挖干净

- **路由**:39 条逐条核对,只有 #497 那一处(已修)。
- **SPEC §9.4 反向**:SPEC 承诺的 29 个接口全部存在,方法也对得上;多出的 10 条是汇总表
  没列但别处有记录的(dev-login、feedback 轮询等)。
- **提示词**:8 份全在用(4 份字面量 + 4 份经 `gradePromptFor` 动态取),没有孤儿。
- **页面**:18 个页面都有站内入口。
...
| 第几次 | 判据 | 结果 | 错在哪 |
| --- | --- | --- | --- |
| 1 | `el === n \|\| n.contains(el) \|\| el.contains(n)` | 0 个点不中 | 把「命中点落在祖先 `<td>` 上」算作通过,真 bug 被判合格 |
| 2 | 去掉 `el.contains(n)` | 14 个点不中 | 13 个是误报:换行的行内链接,中心落在两行间的空隙 |
| 3 | 加「行内链接不算按钮」(抄学生端体检) | 0 个点不中 | 真 bug 也被排掉了 —— 表格操作列里的链接**也是** inline |

一个探针的排除规则,换个上下文就从「去噪」变成「掩盖」。最后靠**直接点一次**拿到地面真相:

```
iPhone 13       390px  「查看整卷」 点不中      Desktop 1280px  ✅
iPhone 13       390px  「发布任务」 点不中      Desktop 1280px  ✅
```

**能直接做那个真动作时,别用近似判据替代它。**

### 修

`overflow-hidden` 从表格挪到外层容器并换成 `overflow-x-auto`:表格在容器里横向滚动,
圆角边框跟着容器走,桌面端一点不变(D77)。同一套表格标记有 5 处,只有试卷列表当前数据
撑到了溢出,另外 4 处是同一个定时炸弹 —— 一并改了,不留「修一个留四个」。

`tests/e2e/teacher-mobile.spec.ts` 守两件事:页面不横向溢出、点了真的跳过去。
空表格的页面不进这个列表 —— 对着空表断言「没有横向溢出」是句废话(D72)。

### 变异验证也写坏了一次

第一版变异把容器的 `overflow-x-auto` 换成 `overflow-hidden`,用例照样全绿 —— 那不是 bug
原状:包了容器后表格是 `w-full`,缩在容器里换行,压根不溢出。改用 `git show HEAD:` 取回
改动前的原版当变异,才验出红(溢出 35px + 点击超时,与最初实测一致)。

这一轮里「变异不忠实」发生了两次(另一次在 #497 的缓存那条)。记法很简单:
**变异跑完要先问「它真复现了那个 bug 吗」,再看红绿。**

### 门禁

- `pnpm lint` / `npx tsc --noEmit` 干净;`pnpm test` **534 passed**(本轮不动被单测覆盖的代码)
- `pnpm build` 通过;e2e 对 **standalone 产物**双视口跑

## 越权(IDOR)逐条核对:18 条带 id 的接口,1 条没收窄(2026-09-17)

`requireTeacherApi` 只校验**角色**,不校验**归属** —— 归属得每条路由自己在查询里做。
这种「靠每个调用点自觉」的设计,漏一条就是一个越权口子,所以逐条核对。

| 范围 | 条数 | 结果 |
| --- | --- | --- |
| 学生作答(`/api/attempts/:id/*`) | 6 | 全部按 `attempts.userId = 我` 收窄 ✅ |
| 教师(任务 / 发布 / 学情 / 导出 / 班级 / 改分 / 学生导出 / 成绩发布) | 8 | 全部按 `classes.teacherId = 我` 或传 `auth.ctx.userId` ✅ |
| 教师(试卷 / 题库 / 小题) | 3 | 全体教师共用,本就不按教师隔离 ✅ |
| `GET /api/teacher/jobs/:id` | 1 | **没有任何归属收窄** ⚠️ |

最后这条只校验角色,然后按 id 把任务原样回出去 —— 连 `grade` 类也回,而那里面是某一个学生的
判分与 AI 中文评语。批改队列按班级收窄(D20)且有用例守着,这条却是条后门。

修法没有加归属列(要迁移,且 parse / explain / generate 本就不该按教师隔离):界面只轮询
那三类,`grade` 的结果由工作线程写回 responses、**从来没有界面轮询过**。按可轮询类型白名单
过滤即可,不可轮询的按 404 处理(403 会确认「这个 id 是条评分任务」)。详见 D78。

严重度说实话:job id 是不可猜的 UUID、只发给入队的那个教师,够不上「能被利用」,这是纵深防御。
但它和一条写明白的隔离决策对不上,而且改起来只是一个白名单。

变异验证:

| 变异 | 期望 | 实际 |
| --- | --- | --- |
| 轮询接口不再按类型过滤(bug 原状:任何类型照回) | 红 | ✅ |
| 把 `grade` 也列进可轮询类型 | 红 | ✅ |

用例里带了三条「扫描不是空的」的对照:评分任务在库里确实存在(只是读不出来)、界面真会轮询的
两类照常拿得到(否则导入向导与题库工具当场坏掉)、不存在的 id 与评分任务返回同一种结果。

### 门禁

- `pnpm lint` / `npx tsc --noEmit` 干净;`pnpm test` **535 passed**(原 534 + 1)
- `pnpm build` 通过;e2e 对 standalone 产物双视口跑

## 参考答案核对,顺手撞上一个能删学生卷子的运维命令(2026-09-17)

这轮换了个维度:不查代码,查**内容正确性**。对题库产品,最坏的静默缺陷不是 bug,
是参考答案错了 —— 学生答对被判错,还不知道为什么。

### 三方核对

原卷 docx(`seed/raw/001_*.docx`)+ 两份独立转写(zsb-qbank 种子、宿主项目的
`exam-hubei-2025.ts`)。原卷**没有答案页**,所以答案只能靠两份转写互证;题干可以对原卷。

机械不变量先过一遍:

- 连词成句 6 题:答案必须正好由题干词块组成,不多不少 → **全过**
- 短文填空带词形提示:答案必须是提示词的变形 → **全过**

再逐题比两份转写的答案键(20 道客观题 + 6 道连词成句):**只有第 22 题分叉**。
宿主接受 `wrong` / `wrongly`,zsb-qbank 只接受 `wrong` —— 学生写 wrongly 会被判错。
按从宽统一,并加 CI 守卫(D80)。阅读问答的差异只是措辞长短(都是给 AI / 老师看的
reference + keyPoints),不算分叉。

### 撞上的东西比原目标严重

核对时顺手看了一眼「改了种子怎么生效」,于是发现:**`pnpm seed` 会把学生的卷子删掉。**
...
种子前：作答 5 行
pnpm seed → 「导入完成」「断言通过」
种子后：attempt 还在，作答 0 行
```

`responses.item_id` 对 `items.id` 是 ON DELETE CASCADE,而种子是整树重建。
「有作答就拒绝重建」那道门只写在 `PUT /api/teacher/papers/:id` 里(D24/D27),
种子脚本直连 `upsertPaper`,什么都继承不到 —— 而 RUNBOOK 恰恰让运维跑 `pnpm seed`。
对一个存着期末考试成绩的系统,这是最坏的一类:静默、无提示、只能靠备份找回。

门搬进 `upsertPaper`(真正删数据的那一层),默认拒绝,`--force` 才毁且先报出将删多少行。
RUNBOOK 补了「改题走教师端小题编辑」的正确做法(D79)。

### 变异验证

| 变异 | 期望 | 实际 |
| --- | --- | --- |
| 去掉整条守卫(bug 原状:种子照删不误) | 红 | ✅ |
| 守卫默认放行(只有显式传 false 才拦) | 红 | ✅ |
| 把第 22 题改回只接受 wrong | 红 | ✅ |
| 改坏另一道题的答案(biggest → bigest) | 红 | ✅ |

### 门禁

- zsb-qbank:`pnpm lint` / `npx tsc --noEmit` 干净;`pnpm test` 全绿;`pnpm build` 通过
- 宿主项目:新增的答案键对齐用例通过

## 运维脚本的两道门:一条能覆盖生产,一条只在生产机上坏(2026-09-17)

接着 D79 那条教训往下查:**还有哪些危险操作的守卫只写在文档或某一个入口上?**
种子那个坑就是这么找到的。这轮扫运维脚本,撞出两个。

### 一、`restore.sh` 少打一个参数 = 拿旧备份盖掉生产

```bash
TARGET="${2:-${DATABASE_URL:?}}"                      # 默认就是生产库
pg_restore --dbname="$TARGET" --clean --if-exists ... # --clean:先删光再灌
```

实测:

```
恢复前：6 行作答（3 条备份前 + 3 条备份之后）
./scripts/restore.sh backups/xxx.dump      ← 少了第二个参数
恢复后：3 行，只剩「备份前」那批
```

无确认、无提示。RUNBOOK 里「务必是另一个库」只是注释。**而恢复恰恰是救火时跑的命令。**
改成:目标库必须显式给;目标=生产还要 `--force`,并先打印它现在装着多少作答(D81)。

### 二、`.env` 盖掉调用方的环境变量,于是恢复演练只在生产机上失败

三个脚本都是 `set -a; . ./.env` 一把梭,`.env` 会盖掉调用方传的值。文档里每条
「环境变量覆盖」的用法都因此失效,其中最要命的是 `verify-restore.sh` 内部那条
`BACKUP_DIR="$WORK/backups" ./scripts/backup.sh` —— 被盖之后 dump 落回 `./backups`,
演练以 `ls: cannot access` 失败。

**`.env.example` 就带着 `BACKUP_DIR=./backups`**:CI 里没有 `.env` 所以一直全绿,
而 RUNBOOK 让 clark 在**有 `.env` 的生产机**上定期跑这条演练。唯一能证明「备份真能
救命」的程序,只在真要用它的那台机器上是坏的(D82)。

抽 `scripts/_env.sh` 把 precedence 摆正(`.env` 只当默认值)。修完在本机带 `.env` 实跑:
**16 张表、208 行逐表一致通过** —— 修复前同一条命令是失败的。

### 变异验证

| 变异 | 期望 | 实际 |
| --- | --- | --- |
| 目标库退回「默认用 DATABASE_URL」(bug 原状) | 红 | ✅ |
| 去掉「目标=生产要 --force」那道门 | 红 | ✅ |
| `.env` 退回一把梭覆盖(调用方的值被盖) | 红 | ✅ |

> 第一条起初**没红**:把默认值加回去之后,第二道门照样拦住了,数据其实仍然安全 ——
> 但我那条用例声称验的是第一道门,却分辨不出这两种情形。改成点名
> 「没有给目标库」这句话之后才验出红。**断言要能分辨出它声称在分辨的东西**(D72 又一次)。
