...
| `POST /api/admin/regrade-phase` | **廉价重评**(评分标准/参照来源改了以后按新标准重评):从 `aiResult` 把感知拷回缓存→置回 UPLOADED 重新入队,重评**复用感知、只重跑判分**(带新 rubric+本人文本参照+合规±10),贵的视频感知一次不重来（body: `schoolId`+`title`+`order`,可选 `limit`） | `schoolId`/`title`/`order` 必填、默认 dry-run 报总盘子+本批、幂等、`objective` 自拒。分批(每次≤100,`more:true` 再 apply 续到排空);`limit` 把本批压小做试点。**只碰已定稿(GRADED/FLAGGED)、非老师改分**的行(老师终评优先,不覆盖);不碰 aiScore/finalScore,由重评落库时覆盖 |
