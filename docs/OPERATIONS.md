...
| `POST /api/admin/reconcile-graded-jobs` | **幽灵死信对账**:把「提交已评出分(aiScore/teacherScore 在)、已落 GRADED/FLAGGED,评阅任务却还挂 FAILED/PENDING」的幽灵任务标记 DONE。成因:已评的行被批量重排后再评又失败到顶,提交留着旧分、任务停在 FAILED,只虚增看板「失败/死信」数(死信数 vs 失败数对不上的根源),老师复核也不清（body: `schoolId`,`title` **选填**） | `schoolId` 必填、默认 dry-run、可反复跑当常备扫帚（幂等，已 DONE 的不再入选）。`title` 不传=全校对账。只碰 FAILED/PENDING（PROCESSING 交给 runner 围栏，不抢）；重评没意义(已有分)、归档更错(有内容有分)——对账 DONE 是唯一正解 |
| `POST /api/admin/set-selection-mode` | **选题落地**:把某作业指定序号的「纯选择环节」(requireChoice+无答案键)标成 `theme`·主题 / `branch`·分流 / `poll`·民调（把历史「投票 hack」正式升格为选题）（body: `schoolId`+`title`+`order`+`mode`） | `schoolId`/`title`/`order`/`mode` 必填、默认 dry-run、可安全重跑（幂等 set-to-value,可逆）。**只写 `Phase.selectionMode`,绝不碰 `Submission`**——学生已选题目+已交+评分无损;不动 `branchTopicsJson`（不追溯给历史作业设分流门）。只认无答案键的环节,不误改客观判分题 |
| `POST /api/admin/set-phase-rubric` | **环节评分标准落地**:把某作业指定序号环节的 `rubric`+`referenceSource`(参照来源:`prior-text`=按本人前置写作环节文本评分;`chunk`=按题库语块的**中心句**评基础分,复述解释句/情景例句各 +10 封顶 +20——整段背诵与逐句跟读都支持,逐句走缓存转写重评不重感知)+`complianceScoring`(背诵检测合规±10) 一次写到该 title 在本校所有班级的同序环节（body: `schoolId`+`title`+`order`+任给 `rubric`/`referenceSource`/`complianceScoring` 其一） | `schoolId`/`title`/`order` 必填、默认 dry-run、幂等。**部分更新**:只写明确给了的字段,省略即不动（不误清历史配置）;`referenceSource` 传 `"prior-text"` 开、`null` 关。**只写 Phase 这三列,绝不碰 Submission/已出评分**——要让新标准生效得走 `regrade-phase` 重评 |
| `POST /api/admin/regrade-phase` | **廉价重评**(评分标准/参照来源改了以后按新标准重评):从 `aiResult` 把感知拷回缓存→置回 UPLOADED 重新入队,重评**复用感知、只重跑判分**(带新 rubric+本人文本参照+合规±10),贵的视频感知一次不重来。重置每行前把旧评分态存进 `regradeSnapshot`(回退安全网)（body: `schoolId`+`title`+`order`,可选 `limit`） | `schoolId`/`title`/`order` 必填、默认 dry-run 报总盘子+本批、幂等、`objective` 自拒。分批(每次≤100,`more:true` 再 apply 续到排空);`limit` 把本批压小做试点。**只碰已定稿(GRADED/FLAGGED)、非老师改分**的行(老师终评优先,不覆盖);不碰 aiScore/finalScore,由重评落库时覆盖 |
| `POST /api/admin/restore-scores` | **重评回退**(看结果不满意一键退回):把某环节被 `regrade-phase` 动过的行按 `regradeSnapshot` 还原成重评前(status/finalScore/aiScore/feedback/needsReview/aiResult),清空快照、作废其重评任务（body: `schoolId`+`title`+`order`,可选 `limit`） | `schoolId`/`title`/`order` 必填、默认 dry-run、幂等(还原过的清了快照、移出谓词)。分批(`more:true` 续)。只还原有快照的行(即被重评动过的);快照在首次重评时捕获、历经多轮重评保住"最初"那份 |
| `POST /api/admin/accept-ai-phase` | **采纳 AI 定稿一个环节**(含 FLAGGED):把某环节「待复核(needsReview,含 FLAGGED 防作弊 + 低置信)且有 AI 分」的提交一次性定稿为已评,`finalScore` 取 `COALESCE(老师分, AI 分)`、清 `needsReview`、置 `GRADED`。期末环节4 决定:录制违规合规已「只奖不罚」并入 AI 分(#425),不再逐份人工复核（body: `schoolId`+`title`+`order`） | `schoolId`/`title`/`order` 必填、默认 dry-run 报盘子(总数 + FLAGGED 占比)、幂等(定稿后移出谓词)。**老师分优先**(COALESCE 不覆盖已改分);只碰待复核+有 AI 分的行,已评/无 AI 分不动。可退 `restore-scores order=N`(会同时撤销本次定稿) |
