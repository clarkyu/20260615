...
| `POST /api/admin/set-phase-rubric` | **环节评分标准落地**:把某作业指定序号环节的 `rubric`+`referenceSource`(参照来源:`prior-text`=按本人前置写作环节文本评分)+`complianceScoring`(背诵检测合规±10) 一次写到该 title 在本校所有班级的同序环节（body: `schoolId`+`title`+`order`+任给 `rubric`/`referenceSource`/`complianceScoring` 其一） | `schoolId`/`title`/`order` 必填、默认 dry-run、幂等。**部分更新**:只写明确给了的字段,省略即不动（不误清历史配置）;`referenceSource` 传 `"prior-text"` 开、`null` 关。**只写 Phase 这三列,绝不碰 Submission/已出评分**——要让新标准生效得走 `regrade-phase` 重评 |
