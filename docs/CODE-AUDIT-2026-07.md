...
| A3 | **S2** | `ai/providers/gemini.ts:120` | 少算 `thoughtsTokenCount` → 默认感知模型每次评阅系统性低报输出成本 | ⬜ |
| A4 | **S2** | `ai/providers/anthropic.ts:42` | Claude 评分 `max_tokens:1024`，长评语截断 → JSON 解析失败 → 提交 FAILED/死信 | ⬜ |
