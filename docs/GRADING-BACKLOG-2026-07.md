...
## 九、评分个性化 · 标准/分值分离与三批采用 AI 收官(2026-07-09)

**背景**:三批需评作业——期末考核(o1 选题 / o2 写作 / o3 朗读 / o4 背诵)与「Native English
2000」五月/六月两批句子背诵训练(o1 逐句跟读 / o2 闭眼背诵)。诉求:三批各环节全部采用 AI 批阅
评分、老师不必逐份复核;所有待批转正归零;把各环节评阅标准落进评分标准里、老师可自由编辑并一键
AI 批阅;**标准(怎么评的文字)与分值(各维度点数)需要分开设置**。

**核心机制(均已合并,PR 台账见文末)**:
- **判分个性化**(#420 PR-1):下游口语环节按「本人前置写作文本」逐句参照(`referenceSource=
  'prior-text'`),而非全班统一参照;背诵检测合规 ±10。
- **廉价重评**(#421 PR-2 `set-phase-rubric` 端点 · #422 PR-3 `regrade-phase` 端点):transcript
  参照无关 → 改标准后**不重跑感知、只重判**(~16× 成本的感知阶段跳过);完整感知存于 `aiResult`,
  重评时恢复。#423 登记 ops 按钮 + 试点 limit。
- **回退安全网**(#424 PR-4 `restore-scores`):重评前存快照,不满意一键还原。
- **合规「只奖不罚」**(#425):录制违规(中途离开/切屏)不再扣分,仅正向加分。
- **语块题库评分**(#427):跟读/背诵只要求中心句为基础分,读背了解释句/情景例句各 +10(累计封顶
  +20、总分封顶 100);逐句跟读复用缓存 `spokenText` 走**文本判分**路径(环节1 不重新感知)。#428
  补 `set-phase-rubric` 放行 `referenceSource='chunk'`。
- **采纳 AI 定稿**(#426 `accept-ai-phase`):把一个环节所有待复核项的 AI 分采纳为终稿(含 FLAGGED),
  `needsReview` 归零、无需老师逐份点。
- **标准/分值分离**(#429 后端 · #430 评分页前端 · #431 作业编辑器前端):
  - 新增 `Phase.rubricPoints`(各维度分值 JSON `[{"name","points"}]`),与 `rubric` 标准文字**分开
    存、分开编辑**;`lib/domain/rubric.ts` 的 `composeRubric` 评分时把两者拼成判分 prompt、**满分取
    各分值之和**(不靠 LLM 算术——分值由代码求和给满分);`parseRubricPoints`/`serializeRubricPoints`
    双向且共用校验。共享组件 `components/rubric-points-editor.tsx`。
  - #430:评分页每环节配置面板加「分值」行编辑器,老师自由增删改 + 一键 AI 批阅(现编标准/分值即时
    生效,`runGrading` 优先取表单分值、回退环节已存),保存可同步到同批次其它班。
  - #431:作业创建/编辑表单也接入同一编辑器(发布时就能分开设);序列化只在 domain 做、repo 只落已
    序列化串(与 `choicesJson`/`blanksJson` 同款,不破分层);模板 payload 往返保留分值(旧模板回退空)。

**三批终态**:各批各环节 `needsReview=0`,全部采用 AI 评分定稿。判分环节分值已回填——期末 o3
(发音25/流利35/内容30/语调10)、o4(完整40/准确20/发音20/流利20)、Native 5月/6月 o2(完整40/
准确20/发音20/流利20),各 9 个班。

**运维**:三批采用 AI / 待批转正 / 回填分值均走 Actions 按钮(`admin-call.yml` 的 `accept-ai-phase`
/ `set-phase-rubric` · `d1-query` 核对),见 `OPERATIONS.md`。**PR 台账**:#420 PR-1 · #421 PR-2 ·
#422 PR-3 · #423 ops · #424 PR-4 · #425 合规 · #426 accept-ai · #427 语块 · #428 chunk 放行 ·
#429 分值分离后端 · #430 评分页分值编辑器 · #431 作业编辑器分值编辑器(均已合并)。
