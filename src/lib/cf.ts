...
export async function runAfterResponse(work: () => Promise<unknown>): Promise<void> {
  try {
    ...
    if (ctx && typeof ctx.waitUntil === 'function') {
      ctx.waitUntil(Promise.resolve().then(work).catch((e) => console.error('[runAfterResponse]', e)))
      return
    }
  } catch {
    // No Cloudflare context (local dev / tests) — fall through to best-effort.
  }
  void Promise.resolve().then(work).catch((e) => console.error('[runAfterResponse]', e))
}
