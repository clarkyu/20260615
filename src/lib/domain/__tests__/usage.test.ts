...
const row = (over: Partial<UsageRow> = {}): UsageRow => ({
  costUsd: 0.03, inputTokens: 15000, outputTokens: 500,
...
describe('summarizeUsage', () => {
  ...
  it('sums cost + tokens and splits by source', () => {
    const s = summarizeUsage([
      row({ costUsd: 0.03, source: 'submission', inputTokens: 15000, outputTokens: 500 }),
      row({ costUsd: 0.01, source: 'practice', inputTokens: 4000, outputTokens: 200 }),
    ])
  ...
  it('groups by teacher, highest spend first', () => {
    const s = summarizeUsage([
      row({ teacherId: 1, teacherName: 'A', costUsd: 0.02 }),
      row({ teacherId: 2, teacherName: 'B', costUsd: 0.05 }),
      row({ teacherId: 1, teacherName: 'A', costUsd: 0.02 }),
    ])
  ...
  it('groups by UTC month, oldest first', () => {
    const s = summarizeUsage([
      row({ at: new Date('2026-08-01T00:00:00Z'), costUsd: 0.02, source: 'practice' }),
      row({ at: new Date('2026-07-15T00:00:00Z'), costUsd: 0.03, source: 'submission' }),
    ])
