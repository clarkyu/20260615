...
export function ChangePasswordForm({ defaultEmail, defaultPhone }: { defaultEmail: string; defaultPhone: string }) {
  const t = useT()
      ...
      <form action={action} className="space-y-4">
        ...
        <div className="space-y-3 border-t border-border/60 pt-4">
          <div className="space-y-1.5">
            ...
            <Input id="email" name="email" type="email" inputMode="email" autoComplete="email" defaultValue={defaultEmail} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="phone">{t('prof.phone')}</Label>
            <Input id="phone" name="phone" type="tel" inputMode="tel" autoComplete="tel" defaultValue={defaultPhone} />
          </div>
          <p className="text-xs text-muted-foreground">{t('change.contactHint')}</p>
        </div>
