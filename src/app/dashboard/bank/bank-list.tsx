...
export function BankList({
  ...
  return (
    <div className="space-y-3">
      ...
      <BankFilters cefr={cefr} strand={strand} domain={domain} series={series} seriesOptions={seriesOptions} />
              ...
              <section key={key || 'ungrouped'} className="overflow-hidden rounded-2xl border border-border/60 bg-card">
                ...
                {open ? (
                  <div className="divide-y divide-border/50 border-t border-border/50">
                    {gs.map((s) => (
                      <Link key={s.id} href={`/dashboard/bank/${s.id}`} className="tap flex items-center gap-3 px-4 py-3 active:bg-secondary/40">
